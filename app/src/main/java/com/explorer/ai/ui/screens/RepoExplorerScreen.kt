    ...
    Column(
        ...
        Column(modifier = Modifier.weight(1f).fillMaxWidth()) {
            ...
            Column(modifier = Modifier.weight(1f).fillMaxWidth().background(Color.Black)) {
                ...
                // Interactive Dock Input Row with QOL Attach, Retry & Clear Chat Buttons
                Row(modifier = Modifier.fillMaxWidth().padding(6.dp), verticalAlignment = Alignment.CenterVertically) {
                    ...
                    if (state.chatHistory.isNotEmpty() && !state.isAiStreaming) {
                        Column(horizontalAlignment = Alignment.CenterHorizontally) {
                            IconButton(onClick = { viewModel.retryLastPrompt() }, modifier = Modifier.size(32.dp)) { Text("🔄", fontSize = 16.sp) }
                            IconButton(onClick = { viewModel.purgeChatHistory() }, modifier = Modifier.size(32.dp)) { Text("🗑️", fontSize = 16.sp) }
                        }
                    }
